...
check_last_run_time() {
  if [ -f "$LAST_RUN_FILE" ]; then
    ...
    if [ "$time_since_last_run" -lt "$TIME_LIMIT_SECONDS" ]; then
      ...
      echo "{BOLD}{RED}Applicaation has been run recently. Please wait $((time_left / 60)) more minutes and $(($time_left % 60)) seconds to re-run.{RESET}"
      exit 1
    fi
  fi
}
...
check_last_run_time
update_last_run_time
echo "{BOLD}{YELLOW}Running app...{RESET}"
